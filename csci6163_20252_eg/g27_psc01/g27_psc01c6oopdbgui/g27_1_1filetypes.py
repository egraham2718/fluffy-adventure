...
class FileType:
    def __init__(self, f_name: str='', 
                 d_path: Path = None):
        self._dirpath: Path = Path(__file__).parent.parent.parent / 'psc01_files' \
            if d_path is None else d_path
...
class SalesFile(FileType):
    def __init__(self, f_name: str = "",
            d_path: Path = None,
            n_convention: str = "sales_qn_yyyy_r.csv") -> None:
        FileType.__init__(self, f_name, d_path)
...
class ImportedFile(FileType):
    def __init__(self, f_name: str = 'imported_files.txt',
