...
def initialize_content_of_files(delimiter: str = ',') -> None:
    ...
    Remove the content of IMPORTED_FILES.
     """
    with (open(FILEPATH / ALL_SALES_COPY, 'r', newline='') as all_sales_copy,
          open(FILEPATH / ALL_SALES, 'w', newline='') as all_sales,
          open(FILEPATH / IMPORTED_FILES, 'w') as imported_files):
        reader = csv.reader(all_sales_copy, delimiter=delimiter)
        writer = csv.writer(all_sales, delimiter=delimiter)
        for row in reader:
            print(f'{row}')
            writer.writerow(row)
...
def import_all_sales() -> list:
    ...
    """
    sales_list = []
    keys = ['amount', 'sales_date', 'region']
    with open(FILEPATH / ALL_SALES, 'r', newline='') as all_sales:
        reader = csv.reader(all_sales)
        for row in reader:
            d = dict(zip(keys, row))
            sales_list.append(d)
